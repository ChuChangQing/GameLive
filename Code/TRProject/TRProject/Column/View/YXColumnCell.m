...
- (YXColumnLinkModel *)linkModel {
    if(_linkModel == nil) {
        _linkModel = [[YXColumnLinkModel alloc] init];
    }
    return _linkModel;
}
- (instancetype)initWithModel:(YXColumnLinkModel *)model{
    if (self = [super init]) {
        ...
        self.nickLb.text = _linkModel.nick;
        self.titleLb.text = _linkModel.title;
        self.onlineLb.text = @(_linkModel.view.integerValue).yx_changeWan;
        [self.iconView sd_setImageWithURL:_linkModel.thumb.yx_URL placeholderImage:nil];
        [self.litView sd_setImageWithURL:_linkModel.avatar.yx_URL placeholderImage:nil];
    }
    return self;
}
@end
